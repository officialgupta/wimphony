...
@app.route('/musearch', methods=['GET', 'POST'])
def musearch():
	if request.method == 'GET':
		field = request.args.get('field')
		q = request.args.get('q')
		...
		if field=="all":
			display = SoundEntry.query.all()
			if display.first() is None:
				display=[]
			return render_template('playbacktemplate.html', songlist=display)
		elif field=="user":
			userid=User.query.filter_by(username=q).first()
			if userid is None:
				return "none"
			...
			return render_template('playbacktemplate.html', songlist=display)
			 ...
			 return render_template('playbacktemplate.html', songlist=[random.choice(SoundEntry.query.all())])
		elif field== "name":
			display = SoundEntry.query.filter_by(name=q)
			if display.first() is None:
				display=[]
			return render_template('playbacktemplate.html', songlist=display)
		return render_template("emptysearchtemplate.html")
